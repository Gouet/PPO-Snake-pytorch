...
def out(inL, padding, dilatation, kernel_size, stride):
    div = inL + 2 * padding - dilatation * (kernel_size - 1) - 1
    div = div / stride + 1
    return div
...
class ActorCriticCNN(torch.nn.Module):
    def __init__(self, num_action, lr, clip_param, final_step, state_shape, continuous=False, std=0.0):
        ...
        self.pi = torch.nn.Sequential(
            torch.nn.Linear(256, self.num_action),
    ...
    def forward(self, x):
        ...
        #conv_out = self.conv(x).view(N, 3200)
